...
def get_ipid(zombie_ip, probe_port, flag="SA"):
    ...
    print("[*] Sending probe to zombie:", pkt.summary())
    ans = sr1(pkt, timeout=7, verbose=0)
    if ans is None:
        print("[!] No reply from zombie (timeout)")
        return None
    if not ans.haslayer(IP):
        print("[!] Reply received but no IP layer:", ans.summary())
        return None
    print("[+] Zombie IPID =", ans[IP].id)
    return ans[IP].id
def trigger_zombie(zombie_ip, target_ip, target_port, ttl):
    spoofed = IP(src=target_ip, dst=zombie_ip, ttl=ttl) / TCP(
        sport=389, # Fixed sport = predictable
        dport=target_port,
        flags="S", # SYN only!
        seq=random.randint(1024, 50000000)
    )
    send(spoofed, verbose=0)
def idle_scan_port(zombie_ip, zport, tip, tport, flag="SA", retries=3, ttl=64):
    for attempt in range(retries):
        ...
        delta = ipid2 - ipid1
        if delta >= 2:
            return f"Check this traffic on wireshark ({delta}) TTL={ttl}"
        elif delta <= 1:
...
for port in target_ports:
    ...
    result = idle_scan_port(zombie_ip, zombie_probe_port, target_ip, port, zombie_tcp_flag, custom_ttl)
    results[port] = result
    print(result)
print("-" * 60)
print("FINAL RESULTS:")
for port, status in results.items():
    print(f" Port {port:5d} → {status}, {str(delta):<6}")
